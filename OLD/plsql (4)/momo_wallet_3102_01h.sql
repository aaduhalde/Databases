...
prompt ****************************************************************
prompt Archivo....: momo_wallet_3102_01h.sql
...
PROMPT ****************************************************************
PROMPT Control de secuencia de carga tabla MOMO_WALLET
PROMPT ****************************************************************
...
BEGIN
	 SELECT TRIM(SUBSTR(min(ARCHIVO),LENGTH('&v_path')+13,11)),
	        TRIM(SUBSTR(min(ARCHIVO),LENGTH('&v_path')+1,43))  
   INTO   v_sin_procesar, v_nombre_archivo
   FROM   G3102_WALLET
   WHERE  ARCHIVO = '&1';

   SELECT TRIM(SUBSTR(max(ARCHIVO),LENGTH('&v_path')+13,11))
   INTO   v_procesado_ok
   FROM   G3102_WALLET
   WHERE  ESTADO IN ('PROCESADO OK','EN PROCESO');

   SELECT (TO_DATE((SELECT TRIM(SUBSTR(MIN(ARCHIVO),LENGTH('&v_path')+13,11)) 
                    FROM G3102_WALLET WHERE  ARCHIVO = '&1'),'YYYYMMDD_HH24') - 
           TO_DATE((SELECT TRIM(SUBSTR(MAX(ARCHIVO),LENGTH('&v_path')+13,11)) 
                    FROM G3102_WALLET WHERE  ESTADO IN ('PROCESADO OK','EN PROCESO')),'YYYYMMDD_HH24')) * 24,
           'momo_wallet_'||TO_CHAR(TO_DATE((SELECT TRIM(SUBSTR(MAX(ARCHIVO),LENGTH('&v_path')+13,11)) 
                                                     FROM G3102_WALLET 
   ...
   IF v_diferencia = 1 OR v_procesado_ok IS NULL THEN
      ...
      WHERE par_procedure_name = 'MOMO_WALLET'
