...
PROMPT Archivo.................: comisiones_psr_C0025_01m.sql
...
/*********************************************************************
              ...
              ----------------------
Se busca el handle de las lineas en la tabla CELLULAR_NUMBER_CHANGES

*********************************************************************/
PROMPT *******************************************************************Procesamiento
PROMPT * * *  Creando la AUX_C0025_PRIM_REC_05_3...
SET TIMING ON
CREATE TABLE AUX_C0025_PRIM_REC_05_3
...
SELECT handle
  ...
  FROM (SELECT /*+ parallel(a,4) parallel(b,4) full(b) */
              ...
              ,b.cnc_start_date
              ,b.cnc_end_date
              ,ROW_NUMBER() OVER(PARTITION BY b.cnc_handle ORDER BY a.fecha_transaccion, b.cnc_start_date DESC) ranking --para rankear los registros de la tabla cellular_number_changes
          FROM aux_c0025_prim_rec_05_2 a
              ,&v_esquema_rac8.cellular_number_changes b
         WHERE a.cellular_number = b.cnc_clu_cellular
           AND a.fecha_transaccion >= b.cnc_start_date
           AND a.fecha_transaccion < NVL(b.cnc_end_date, TO_DATE('31/12/3999', 'dd/mm/yyyy')))
 WHERE ranking = 1
/
SET TIMING OFF
