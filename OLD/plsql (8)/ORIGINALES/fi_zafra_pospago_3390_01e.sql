prompt ****************************************************************
prompt Archivo....: fi_zafra_pospago_3390_01e.sql
prompt Autor......: Leandro Mestrallet
prompt
prompt Descripción: Actualizo tabla final de migraciones prepago a pospago.
prompt
prompt ****************************************************************
...
prompt ****************************************************************
prompt ELIMINA DATOS DE B_FI_ZAFRA_MIG_POSP POR REPROCESO
prompt ****************************************************************
DECLARE
  partition_not_exist EXCEPTION;
  PRAGMA EXCEPTION_INIT(partition_not_exist, -2149);

  fecha DATE := TO_DATE('&V_ANIOMES_ACT', 'yyyymm');

BEGIN
  dbms_output.put_line('alter table B_FI_ZAFRA_MIG_POSP truncate partition for (to_date(''' ||
                       to_char(fecha, 'dd/mm/yyyy') ||
                       ''',''dd/mm/yyyy''))');
  execute immediate 'alter table B_FI_ZAFRA_MIG_POSP truncate partition for (to_date(''' ||
...
/
prompt ****************************************************************
prompt ACTUALIZA TABLA FINAL DE MIGRACIONES B_FI_ZAFRA_MIG_POSP
prompt ****************************************************************
INSERT /*+ APPEND PARALLEL(b,4)*/  
INTO B_FI_ZAFRA_MIG_POSP 
(
FECHA,
ANIO_MES,
LINEA_CB,
...
where ACT_TABLE_NAME= 'B_FI_ZAFRA_MIG_POSP'
/
COMMIT
/
