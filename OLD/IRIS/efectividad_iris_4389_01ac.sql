...
prompt ****************************************************************
prompt Archivo....: efectividad_iris_4389_01ac.sql 
...
PROMPT ****************************************************************
PROMPT ****************************************************************
PROMPT Creando AUX_G4389_IRIS_27
PROMPT ****************************************************************

create TABLE AUX_G4389_IRIS_27
...
SELECT /*+ parallel(OPI,4) */
  ...
  OPI.PPI_CLU_CELLULAR_NUMBER LINEA_CLIENTE, 
  MIN(OPI.PPI_CTY_ID) ID_PACK, 
  OPI.PPI_ADD_DATE FECHA_COMPRA_PACK, 
  OPI.PPI_USR_ID CANAL_PACK  
...
AND OPI.PPI_ADD_DATE =
                        ( SELECT MIN(OPI.PPI_ADD_DATE)
                               FROM  AUX_G4389_IRIS_24 I24AUX       
                               JOIN &v_esquema_rac8.OL_PREPAY_INTERFACES OPI
                                  ON (I24AUX.CELLULAR_NUMBER_CLI = OPI.PPI_CLU_CELLULAR_NUMBER)
                        WHERE   PPI_PPS_ID = 'PF'
                         AND PPI_IST_ID = 'Y'
                         AND TRIM (PPI_GENERIC_DATA) IS NOT NULL
                         AND TRIM (PPI_GENERIC_DATA) NOT IN ('P_PACK_PP=Y')
                         AND TRUNC(PPI_ADD_DATE) = '&v_fecha_proceso'
                         AND PPI_ADD_DATE BETWEEN I24AUX.FECHA_HORA_RECARGA AND (I24AUX.FECHA_HORA_RECARGA + 3/24)
                         AND OPI.PPI_CLU_CELLULAR_NUMBER = I24.CELLULAR_NUMBER_CLI
                        )
GROUP BY 
  I24.FECHA_RECARGA,
  I24.CELLULAR_NUMBER_CLI,
