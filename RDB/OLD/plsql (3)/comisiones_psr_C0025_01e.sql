...
PROMPT Archivo.................: comisiones_psr_C0025_01e.sql
...
PROMPT * * *  Creando la AUX_C0025_PRIM_REC_02...
SET TIMING ON
CREATE TABLE AUX_C0025_PRIM_REC_02
...
SELECT /*+ parallel(a,4) parallel(b,4) */
      ...
      ,b.account_id
      ,b.cbt_id
      ,b.treatment_id promo
      ,a.origen
      ,a.origen_trx
  FROM aux_c0025_prim_rec_01 a
      ,&v_esquema_rac8.cellulars b
 WHERE a.bill_number = b.bill_number
   AND a.origen_trx <> 3
   AND b.treatment_id IN '&v_promo'
UNION ALL
SELECT /*+ parallel(a,4) parallel(b,4) */
       a.fecha_recarga
      ,a.tt
  ...
  FROM aux_c0025_prim_rec_01 a
      ,&v_esquema_rac8.cellulars b
 WHERE a.bill_number = b.cellular_number
   AND a.origen_trx = 3
   AND b.treatment_id IN '&v_promo'
/
SET TIMING OFF
