...
prompt ****************************************************************
prompt Archivo....: momo_scratchcards_history_3440_01d.sql
...
SELECT pck_core.fc_get_parameter('CORE','TBS_ADMIN') tbs_admin,
       ...
       pck_core.fc_get_parameter('MOMO_SCRATCHCARDS_HISTORY','PATH_SCRIPT') path_script,
...
CREATE TABLE AUX_3440_SECUENCIA_EJECUCION
TABLESPACE &v_tbs_admin PARALLEL 4 AS
SELECT ARCHIVO, 
       'start '||'&v_path_script'||'momo_scratchcards_history_3440_01h.sql'|| 
       ' '||TRIM(ARCHIVO) AS SCRIPT_SQL                                  --VALOR DEL CAMPO ARCHIVO
FROM   G3440_SCRATCHCARDS_HISTORY
WHERE  ESTADO = 'SIN PROCESAR'
UNION
SELECT ARCHIVO, 
       'start '||'&v_path_script'||'momo_scratchcards_history_3440_01i.sql'||    
       ' AUX_3440_'||TRIM(SUBSTR(ARCHIVO,LENGTH('&v_path')+27,11))||     --NOMBRE DE LA TABLA AUXILIAR
       ' '|| TRIM(SUBSTR(ARCHIVO,LENGTH('&v_path')+1,41)) AS SCRIPT_SQL  --NOMBRE ARCHIVO PLANO
FROM   G3440_SCRATCHCARDS_HISTORY
WHERE  ESTADO = 'SIN PROCESAR'
UNION 
SELECT ARCHIVO, 
       'start '||'&v_path_script'||'momo_scratchcards_history_3440_01j.sql'||  
       ' AUX_3440_'||TRIM(SUBSTR(ARCHIVO,LENGTH('&v_path')+27,11))||     --NOMBRE DE LA TABLA AUXILIAR 
       ' '|| TRIM(SUBSTR(ARCHIVO,LENGTH('&v_path')+1,41))||              --NOMBRE ARCHIVO PLANO
       ' '||TRIM(ARCHIVO) AS SCRIPT_SQL                                  --VALOR DEL CAMPO ARCHIVO
FROM   G3440_SCRATCHCARDS_HISTORY
WHERE  ESTADO = 'SIN PROCESAR'
/
...
spoo &v_path_script.momo_scratchcards_history_3440_01e.sql

SELECT SCRIPT_SQL  
FROM   AUX_3440_SECUENCIA_EJECUCION 
ORDER BY ARCHIVO, SCRIPT_SQL
/
	 
spoo OFF
! chmod 755 &v_path_script.momo_scratchcards_history_3440_01e.sql
/
