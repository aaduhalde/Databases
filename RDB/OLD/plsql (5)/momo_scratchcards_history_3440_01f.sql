...
prompt ****************************************************************
prompt Archivo....: momo_scratchcards_history_3440_01f.sql
...
CREATE TABLE AUX_3440_SECUENCIA_EJECUCION_2
TABLESPACE &v_tbs_admin PARALLEL 4 AS
SELECT ARCHIVO,
       'start '||'&v_path_script'||'momo_scratchcards_history_3440_01k.sql'||              
       ' AUX_3440_'||TRIM(SUBSTR(ARCHIVO,LENGTH('&v_path')+28,10)) AS SCRIPT_SQL   --NOMBRE DE LA TABLA AUXILIAR
FROM   G3440_SCRATCHCARDS_HISTORY
WHERE  ESTADO IN ('EN PROCESO','SIN PROCESAR')
UNION
SELECT ARCHIVO,
       'start '||'&v_path_script'||'momo_scratchcards_history_3440_01l.sql'|| 
...
set linesize 500
set timing off
spoo &v_path_script.momo_scratchcards_history_3440_01g.sql

SELECT SCRIPT_SQL  
FROM   AUX_3440_SECUENCIA_EJECUCION_2
ORDER BY ARCHIVO, SCRIPT_SQL
/
	 
spoo OFF
! chmod 755 &v_path_script.momo_scratchcards_history_3440_01g.sql
/
