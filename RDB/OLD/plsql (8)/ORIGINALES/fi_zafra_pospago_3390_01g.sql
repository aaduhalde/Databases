prompt ****************************************************************
prompt Archivo....: fi_zafra_pospago_3390_01g.sql
prompt Autor......: Leandro Mestrallet
prompt
prompt Descripción: Actualizo tablas final detallada de migraciones.
prompt
prompt ****************************************************************
...
prompt ****************************************************************
prompt ELIMINA DATOS DE B_FI_ZAFRA_POSP_MIG POR REPROCESO
prompt ****************************************************************
DECLARE
...
BEGIN
  dbms_output.put_line('alter table B_FI_ZAFRA_POSP_MIG truncate partition for (to_date(''' ||
                       to_char(fecha, 'dd/mm/yyyy') ||
                       ''',''dd/mm/yyyy''))');
  execute immediate 'alter table B_FI_ZAFRA_POSP_MIG truncate partition for (to_date(''' ||
...
/
prompt ****************************************************************
prompt ACTUALIZA TABLA DE ACTIVACIONES B_FI_ZAFRA_POSP_MIG
prompt ****************************************************************
INSERT /*+ APPEND PARALLEL(b,4)*/
INTO B_FI_ZAFRA_POSP_MIG
 (
 	FECHA,
  ANIO_MES,
  CELLULAR_NUMBER,
...
where ACT_TABLE_NAME= 'B_FI_ZAFRA_POSP_MIG'
/
COMMIT
/
